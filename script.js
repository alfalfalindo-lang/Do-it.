...
function criarElementoTarefa(t, fonte){
  ...
  const dataLabel   = t.data ? `<span class="tarefa-data">${formatarData(t.data)}</span>` : "";
  ...
  div.innerHTML = `
    ...
    <div class="tarefa-info">
      ...
      <div class="tarefa-meta">${dataLabel}${catLabel}${recLabel}</div>
    </div>
...
document.querySelectorAll(".filtro-cor-btn").forEach(btn => {
  btn.onclick = () => {
    ...
    document.querySelectorAll(".filtro-cor-btn").forEach(b=>b.classList.remove("active"));
    btn.classList.add("active");
    render();
  };
});
...
document.querySelectorAll(".cor").forEach(btn => {
  btn.onclick = () => {
    ...
    document.querySelectorAll(".cor").forEach(b=>b.classList.remove("selected"));
...
document.getElementById("criar").onclick = () => {
  ...
  const obj = {
    ...
    recorrente, frequencia: recorrente ? document.getElementById("frequencia").value : null
  };
...
function mostrarSeletorPostit(x,y){
  ...
  sel.style.left = (x-20)+"px";
  sel.style.top  = (y+10)+"px";
  document.body.appendChild(sel);
  sel.querySelectorAll(".postit-cor-op").forEach(op => {
    op.onclick = () => {
      const obj = {top:"180px",left:"340px",texto:"",classe:op.dataset.classe};
      postits.push(obj); criarPostit(obj); salvar(); sel.remove();
    };
  });
  setTimeout(() => {
    document.addEventListener("click", function f(ev){ if(!sel.contains(ev.target)){sel.remove();document.removeEventListener("click",f);} });
  }, 50);
}
...
function criarPostit(p){
  ...
  div.querySelector(".postit-handle").onmousedown = e => {
    ...
    document.onmousemove = e => {
      ...
      const sobre=px.right>lx.left&&px.left<lx.right&&px.bottom>lx.top&&px.top<lx.bottom;
      document.getElementById("lixeira").classList.toggle("highlight",sobre);
    };
    document.onmouseup = () => {
